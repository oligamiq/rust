...
impl Step for CodegenBackend {
    ...
    fn run(self, builder: &Builder<'_>) {
        ...
        let mut cargo = builder.cargo(compiler, Mode::Codegen, SourceType::Submodule, target, "build");
...
fn copy_codegen_backends_to_sysroot(builder: &Builder<'_>,
    ...
    for backend in builder.config.rust_codegen_backends.iter() {
        ...
        // change `librustc_codegen_cranelift-xxxxxx.so` to `librustc_codegen_cranelift.so`
        let target_filename = {
